...
// 导入词库管理工具和发音功能
import { getWordbookById, getSectionWords, getRandomWords } from '@/lib/utils/wordbook-manager'
...
// 章节ID映射：根据词书ID获取默认章节
const getDefaultSectionId = (wordbookId: string): string => {
...
export default function StudyPageClient({ id, listId, mode }: StudyPageClientProps) {
  const router = useRouter()
  // 加载真实单词数据
  const words = useMemo(() => {
    try {
      const actualWordbookId = WORDBOOK_ID_MAP[id] || id
      const actualSectionId = listId.match(/^\d+$/)
        ? getDefaultSectionId(actualWordbookId)
        : listId
      const rawWords = getSectionWords(actualWordbookId, actualSectionId)
      ...
      // Get SRS progress
      const progress = getListProgress(actualWordbookId, actualSectionId)
      ...
      if (isFirstStudy(progress)) {
        return rawWords.map(adaptWord)
      }

      // Otherwise, only show words that are due for review
      const dueWordIds = getDueWords(progress)
  ...
  }, [id, listId, mode, router])

  const [currentIndex, setCurrentIndex] = useState(0)
  const [userInput, setUserInput] = useState('')
  const [selectedOption, setSelectedOption] = useState<number | null>(null)
  const [matchingPairs, setMatchingPairs] = useState<{ left: number[], right: number[] }>({ left: [], right: [] })
  ...
  const handleRating = (rating: 'again' | 'hard' | 'good' | 'easy') => {
    ...
    let progress = getListProgress(actualWordbookId, actualSectionId)
    if (!progress) {
      const allWordIds = words.map(w => w.id)
      progress = initializeListProgress(actualWordbookId, actualSectionId, allWordIds)
    }

    // Update word progress with SRS algorithm
    progress = updateWordProgress(progress, currentWord.id, rating)

    // Check if this is the last word in first study
    if (isFirstStudy(progress) && currentIndex === words.length - 1) {
      progress = markFirstStudyCompleted(progress)
    }

    // Save progress
    saveListProgress(progress)
  ...
  const handleDictationSubmit = () => {
    ...
    progress = markWordForReview(progress, currentWord.id, isCorrect)
    saveListProgress(progress)
  ...
  const handleMatchingClick = (index: number) => {
    ...
    if (firstItem.id === secondItem.id) {
      ...
      progress = markWordForReview(progress, firstItem.id, true)
      saveListProgress(progress)
      ...
      progress = markWordForReview(progress, firstItem.id, false)
      saveListProgress(progress)
